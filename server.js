...
update = function()
  ...
  if keyboard.press.M then
    if game_state == "play" and is_admin == 1 then
      connection.send(object mtype="admin_double" end)
    end
  end
...
update_game = function()
  ...
  // Kollision mit Coins (rueckwaerts fuer sicheres removeAt)
  local i = coins.length - 1
  while i >= 0
    local c = coins[i]
    if c != 0 then
  ...
  // Explosionen updaten (rueckwaerts)
  local j = explosions.length - 1
  while j >= 0
...
draw_game = function()
  ...
  if is_admin == 1 then
    screen.drawText("ADMIN [M] Score x2", 0, -screen.height/2 + 40, 10, "red")
  end
...
serverUpdate = function()
  ...
  for message in server.messages
    if message.data != 0 then
      ...
      if message.data.mtype == "register" then
        ...
        if players[pid] != 0 and players[pid] != null and players[pid].logged_in == 1 then
          ...
          for bc in server.active_connections
            bc.send(object mtype="bomb" coin_id=coin_id bx=bomb_bx bomb_by=bomb_by victim=victim lives=players[pid].lives totems=players[pid].totems gameover=is_gameover end)
          end
          // Bei Game Over resetten
          if is_gameover then
            players[pid].score = 0
        ...
        if players[pid] != 0 and players[pid] != null and players[pid].logged_in == 1 then
          if message.data.volume != null then
            players[pid].volume = message.data.volume
          end
          if message.data.skin != null and message.data.skin >= 1 and message.data.skin <= 3 then
            players[pid].skin = message.data.skin
          end
          save_player_to_db(pid)
          send_player_data(message.connection, pid)
          broadcast_all()
      ...
      elsif message.data.mtype == "admin_double" then
        if players[pid] != 0 and players[pid] != null and players[pid].logged_in == 1 then
          if players[pid].is_admin == 1 then
            players[pid].score = players[pid].score * 2
            save_player_to_db(pid)
            send_player_data(message.connection, pid)
            broadcast_all()
          end
  ...
  // Coins bewegen und entfernen (rueckwaerts iterieren fuer sicheres removeAt)
  local i = coins.length - 1
  while i >= 0
    if coins[i] != 0 then
  ...
  local raw_rate = 40 - pcount * 5
  local spawn_rate = 10
  if raw_rate > 10 then spawn_rate = raw_rate end
